...
{
    private static TransHelper _instance;
    public static TransHelper Instance => _instance ?? (_instance = new TransHelper());
    ...
    void Trans(Entity entity)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (GameData.Instance.posToEntity.TryGetValue(p1, out var es1) && GameData.Instance.posToEntity.TryGetValue(p2, out var es2))
            {
